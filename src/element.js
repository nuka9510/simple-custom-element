...
export default class SceElement extends HTMLElement {
	...
	constructor() {
		...
		this.#root = SceContext.getRoot(this);
	...
	/** @type {sce_element.SceElement['_eventCancel']} */
	#eventCancel(ev) { ev.preventDefault(); }

	/** @type {sce_element.SceElement['_bubbleStop']} */
	#bubbleStop(ev) { ev.stopPropagation(); }
	...
	#subSelect(ev) {
		...
		subNode = this.querySelectorAll(`select[data-sce-id="${node.dataset.sceTarget}"]`);

		subNode.forEach(async (el, i, arr) => {
